...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <form class="form-horizontal" action="/admin/workers/{{$workers->id}}" method="post"
                      enctype="multipart/form-data">
                    @method("PUT")
                    @csrf
                    <div class="card-body">
                        <h4 class="card-title">Edit The Worker</h4>
                        <div class="form-group row">
                            <label for="Name" class="col-sm-3 text-right control-label col-form-label">Name</label>
                            <div class="col-sm-9">
                                <input type="text" required class="form-control" id="Name" placeholder="Name"
                                       name="name" value="{{$workers->name}}">
                                @error('name')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" required class="form-control" id="Surname" placeholder="Surname"
                                       name="surname" value="{{$workers->surname}}">
                                @error('surname')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" required class="form-control" id="position" placeholder="Position"
                                       name="position" value="{{$workers->position}}">
                                @error('position')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-sm-9">
                                <select class="select2 form-control custom-select" id="parent" name="parent_id" required>
                                    @foreach($blogs as $key => $value)
                                        <option @if($value->id === $workers->parent_id) selected @endif
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="number" required class="form-control" id="Order" placeholder="Order"
                                       value="{{$workers->order}}" name="order">
                                @error('order')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
